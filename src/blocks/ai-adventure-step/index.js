...
registerBlockType( metadata.name, {
	edit: ( { attributes, setAttributes, clientId } ) => {
		...
		const availableSteps = useSelect( ( select ) => {
			...
			// Add special options
			const options = [
				{ value: '', label: __('Select destination...', 'extrachill-blog') },
				{ value: 'end_game', label: __('🏁 End Game', 'extrachill-blog') }
			];
			if (steps.length > 0) {
				options.push(...steps);
			}
		...
		return (
			<>
				<InspectorControls>
					<PanelBody title={ __( 'Story Triggers', 'extrachill-blog' ) }>
						<p className="components-base-control__help">
							{ __( 'Define semantic conditions that advance the story. Use descriptive phrases like "Player opens the chest" or "Player talks to the wizard".', 'extrachill-blog' ) }
						</p>
						{ triggers.map( ( trigger, index ) => (
							<div key={ index } className="trigger-item" style={ { marginBottom: '20px', padding: '15px', border: '1px solid #ddd', borderRadius: '4px' } }>
								<TextareaControl
									label={ __( 'Trigger Condition', 'extrachill-blog' ) }
									...
									help={ __( 'Describe the player action or decision that should trigger this path.', 'extrachill-blog' ) }
									rows={ 3 }
								/>
								<SelectControl
									label={ __( 'Destination', 'extrachill-blog' ) }
									value={ trigger.destinationStep }
									onChange={ ( value ) => handleTriggerChange( value, index, 'destinationStep' ) }
									options={ availableSteps }
									help={ __( 'Where should the story go when this trigger is activated?', 'extrachill-blog' ) }
								/>
								<Button isLink isDestructive onClick={ () => removeTrigger( index ) }>
									{ __( 'Remove Trigger', 'extrachill-blog' ) }
								</Button>
							</div>
						) ) }
						<Button variant="primary" onClick={ addTrigger }>
							{ __( 'Add Story Trigger', 'extrachill-blog' ) }
						</Button>
					</PanelBody>
				</InspectorControls>
				<div { ...useBlockProps() }>
					<RichText
						...
						value={ stepPrompt }
						placeholder={ __( 'Step Action: What is happening at this moment in the story?', 'extrachill-blog' ) }
					/>
				</div>
			</>
		);
