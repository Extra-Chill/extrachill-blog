...
registerBlockType( metadata.name, {
	edit: ( { attributes, setAttributes, clientId } ) => {
		const blockProps = useBlockProps({ className: 'extrachill-block-window' });
		...
		return (
			<>
				<InspectorControls>
					<PanelBody title={ __( 'AI Settings', 'extrachill-blog' ) }>
						<TextareaControl
							label={ __( 'Game Master Persona', 'extrachill-blog' ) }
							value={ gameMasterPersona }
							onChange={ ( value ) => setAttributes( { gameMasterPersona: value } ) }
							help={ __( 'Define the personality and role of the AI storyteller.', 'extrachill-blog' ) }
						/>
					</PanelBody>
				</InspectorControls>
				<div { ...blockProps }>
					<RichText
						...
						value={ title }
						placeholder={ __( 'Enter Adventure Title', 'extrachill-blog' ) }
					/>
					<RichText
						tagName="p"
					...
					/>
					<InnerBlocks
						allowedBlocks={ [ 'extrachill/ai-adventure-path' ] }
					/>
				</div>
			</>
		);
